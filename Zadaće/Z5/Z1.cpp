...
class HashMapaLan: public Mapa<TipKljuca,TipVrijednosti>{
    ...
    public:
    HashMapaLan():velicina(0),kapacitet(2),hash_map(kapacitet,list<pair<TipKljuca,TipVrijednosti>>()),hashFunkcija(nullptr){}
    ~HashMapaLan(){}
    void definisiHashFunkciju(unsigned int(*hashFunction)(TipKljuca kljuc, unsigned int opseg)){
        hashFunkcija=hashFunction;
    }
    int brojElemenata()const{return velicina;}
    void obrisi(){
        for(auto &element: hash_map) element.clear();
        velicina=0;
    }
    void obrisi(const TipKljuca &kljuc){
        if(velicina==0) return;
        if(!hashFunkcija) throw logic_error("Nije definisana hash funkcija!");
        ...
        if(it==hash_map[pronadji].end() || it->first!=kljuc) throw logic_error("Trazeni element sa tim kljucem nije pronadjen!");
        auto kopija=it;
        it++;
    ...
    TipVrijednosti &operator[](TipKljuca kljuc){
        ...
        while(it!=hash_map[pronadji].end() && kljuc>it->first) it++;
        if(it==hash_map[pronadji].end() || it->first!=kljuc){
            it=hash_map[pronadji].insert(it,pair<TipKljuca,TipVrijednosti>(kljuc,tp));
            velicina++;
        }
        return it->second;
    }
    TipVrijednosti operator[](const TipKljuca kljuc) const{
        ...
        TipVrijednosti tp=TipVrijednosti();
        while(it!=hash_map[pronadji].end() && kljuc>it->first) it++;
        if(it==hash_map[pronadji].end() || it->first!=kljuc) tp;
        return it->second;
    }
};

//PZ10 prekopirana u Z5
...
class GranaIterator{
    ...
    void inkrementuj(int &polazni_cvor, int &dolazni_cvor){
        dolazni_cvor+1>=ug->dajBrojCvorova() ? dolazni_cvor=0, polazni_cvor++ : dolazni_cvor++;
    }
    public:
...
struct Element{
    ...
    int vrijednost; 
    TipOznake oznaka;
    Element(int vertex, float weight=0, TipOznake oznaka=TipOznake()):vrijednost(vertex),tezina(weight),oznaka(oznaka){}
};
template <typename TipOznake>
class ListaGraf: public UsmjereniGraf<TipOznake>{
    ...
    ListaGraf(int n):UsmjereniGraf<TipOznake>(n){
        if(n<0) throw logic_error("Lista susjedstva ne moze imati negativan broj elemenata!");
        elementi.resize(n);
        cvorovi.resize(n);
    }
    ~ListaGraf(){}
    ...
    //Grane metode
    typename list<Element<TipOznake>>::iterator trazeni(int polazni_cvor, int dolazni_cvor){
        typename list<Element<TipOznake>>::iterator it;
        for(it=elementi[polazni_cvor].begin();it!=elementi[polazni_cvor].end();it++){
            if(it->vrijednost==dolazni_cvor) return it;
            else if(it->vrijednost>dolazni_cvor) return elementi[polazni_cvor].end(); 
        }
        return it;
    }
    typename list<Element<TipOznake>>::const_iterator trazeni(int polazni_cvor,int dolazni_cvor)const{
    ...
    void dodajGranu(int polazni_cvor, int dolazni_cvor, float tezina_grane){
        ...
        while(it!=elementi[polazni_cvor].end() && dolazni_cvor>it->vrijednost) it++;
        elementi[polazni_cvor].insert(it, Element<TipOznake>(dolazni_cvor,tezina_grane));
    }
...
void dfs(UsmjereniGraf<TipOznake> *ug, vector<Cvor<TipOznake>> &depth, Cvor<TipOznake> cvor){
    ...
    while(breach!=ug->dajGraneKraj()){
        ...
        if((*breach).dajPolazniCvor().dajRedniBroj()==cvor.dajRedniBroj() && pom.dajOznaku()!=1) 
            dfs(ug,depth,pom);
        breach++;
    }
}
