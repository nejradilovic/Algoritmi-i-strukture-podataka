...
struct Cvor{
    ...
    int balans_cvora=0; //dodajemo balans kao sto je receno u postavci zadace
   // Cvor():roditelj(nullptr),lijevo_podstablo(nullptr),desno_podstablo(nullptr){}
...
class AVLStabloMapa: public Mapa<TipKljuca,TipVrijednosti>{
    ...
    Cvor<TipKljuca,TipVrijednosti> *korijen=nullptr;
    public:
    //Konstruktori, operatori i destruktor
    ...
    //Rotacije u svrhu balansiranja stabla
    /*Postoje dva tipa rotacije kod AVL stabla: jednostruka i dvostruka(dvije jednostruke rotacije u suprotnim smjerovima)
    Ako kriticni cvor i njegovo dijete naginju na istu stranu vrsimo jednostruku rotaciju, lijevu ako naginju na desnu a desnu ako naginju na lijevu 
    Ako ne naginju na istu stranu primjenjujemo dvostruku rotaciju*/
    ...
    //Narusen balans zahtjeva da ga obnovimo, ako je cvor bez roditelja on je korijeni te se rekurzija prekida
    ...
    //Moramo promijeniti balans iz razloga sto brisemo
    void obrisi(const TipKljuca &ukloniti){
        Cvor<TipKljuca, TipVrijednosti> *p=korijen, *q=nullptr, *s=nullptr;
        while(p!=nullptr && (p->kljuc)!=ukloniti){
            q=p;
...
int main(){
    ...
    time2_bsm=clock();
    //Umetanje 10000 elemenata u AVLStabloMapa
    time1_avl=clock();
    ...
    bsm[10001]=rand();
    time4_bsm=rand();
    //Umetanje novog elementa u AVLStabloMapa
    time3_avl=clock();
    avl[10001]=rand();
    time4_avl=clock();
    ...
    bsm[5788];
    time6_bsm=clock();
    //Pristup postojecem elementu u AVLStabloMapa
    time5_avl=clock();
    avl[5788];
    time6_avl=clock();
    ...
    AVL moze ubaciti veliki broj elemenata brzo i brzo pronalazi element. AVL je samobalansirajuce stablo i kod njega je visina sto moguce manja,
    te je vremenska slozenost svih operacija bolja kod AVL stabla nego kod BinStabla jer je worst case svih operacija kod AVL stabla O(nlogn) dok je kod BinStabla O(n)*/
